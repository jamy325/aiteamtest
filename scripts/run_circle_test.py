...
from services.dxf_exporter import DxfExporter, ExportMode as DxfExportMode
...
from services.svg_exporter import ExportMode as SvgExportMode
...
def create_synthetic_circle_images(input_dir: str | Path) -> tuple[Path, ...]:
    ...
    existing = _image_files(directory)
    if existing:
        return existing
...
def run_circle_tests(
    *,
    ...
    results: list[CircleTestRunResult] = []
    for image_path in images:
        result = run_single_circle_test(
    ...
    if fail_on_no_circle and any(not bool(item.summary["has_circle_segment"]) for item in results):
        raise SystemExit(1)
...
def run_single_circle_test(
    *,
    ...
    sampler: SegmentSampler | None = None,
    export_mode: str = "centerline",
    debug: bool = True,
    min_path_area: float = 256.0,
    prefer_source: str = "skeleton_contour",
    ...
    image_output_dir = Path(output_root) / image_path.stem
    image_output_dir.mkdir(parents=True, exist_ok=True)

    pipeline = pipeline or MinimalPipeline(segment_type="line")
    preview_service = preview_service or CommandPreviewService()
    command_executor = command_executor or CommandExecutor()
    integrity_validator = integrity_validator or DocumentIntegrityValidator()
    svg_exporter = svg_exporter or SvgExporter()
    dxf_exporter = dxf_exporter or DxfExporter()
    sampler = sampler or SegmentSampler()

    document_id = f"manual_circle_test_{image_path.stem}"
    debug_output_root = image_output_dir / "debug" if debug else None
    ...
    command = None if selected is None else {
    ...
    if command is not None:
        ...
        execution_result = command_executor.execute(command, before_document)

    after_document = before_document
    execute_success = False
    execute_reason: str | None = None
    if execution_result is None:
        execute_reason = "no suitable closed path candidate found"
    else:
        execute_success = execution_result.success
        execute_reason = execution_result.reason
        if execution_result.success:
            after_document = execution_result.document

    integrity_report = integrity_validator.validate(after_document)

    before_json_path = image_output_dir / "vector_document_before.json"
    after_json_path = image_output_dir / "vector_document_after.json"
    overlay_before_path = image_output_dir / "overlay_before.png"
    overlay_after_path = image_output_dir / "overlay_after.png"
    svg_path = image_output_dir / "vector_result.svg"
    dxf_path = image_output_dir / "vector_result.dxf"
    summary_path = image_output_dir / "summary.json"
    vision_manifest_path = image_output_dir / "vision_manifest.json"

    before_json_payload = pipeline.export_json(before_document, before_json_path)
    after_json_payload = pipeline.export_json(after_document, after_json_path)
    ...
    svg_payload = svg_exporter.export_document(after_document, export_mode=_coerce_svg_export_mode(export_mode))
    svg_path.write_text(svg_payload, encoding="utf-8")
    dxf_payload = dxf_exporter.export_document(after_document, export_mode=_coerce_dxf_export_mode(export_mode))
    ...
    selected_path_before = None if selected is None else _path_by_id(before_document, selected.path_id)
    output_files = {
    ...
    debug_output_dir = (
        str(pipeline_result.debug_artifacts.output_dir)
        if pipeline_result.debug_artifacts is not None
        else None
    )
    if pipeline_result.debug_artifacts is not None:
        output_files["debug_output_dir"] = debug_output_dir
    ...
    summary = {
        ...
        "selected_path_segment_count_before": None if selected_path_before is None else len(selected_path_before.segments),
        ...
        "integrity_success": integrity_report.success,
        "integrity_errors": [
            {
                "code": issue.code,
            ...
            for issue in integrity_report.errors
        ],
        ...
        "debug_files": (
            list(pipeline_result.debug_artifacts.exported_files)
            if pipeline_result.debug_artifacts is not None
            else []
        ),
        "command": None if command is None else dict(command),
        "preview_reason": None if preview_result is None else preview_result.reason,
        "before_json_char_count": len(before_json_payload),
        "after_json_char_count": len(after_json_payload),
    }
...
def rank_circle_path_candidates(
    ...
    *,
    ...
    allowed_sources = {prefer_source, "binary_contour", "skeleton_contour"}

    for path in document.paths:
        sampled_points = sample_path_points(document, path, sampler=sampler)
        bbox = path_bbox(sampled_points)
        ...
        if path.source not in allowed_sources:
            ...
            score = _candidate_score(
                ...
                bbox=bbox,
                bbox_area=bbox_area,
                aspect_ratio=aspect_ratio or 1.0,
                document=document,
                prefer_source=prefer_source,
            )
...
def select_circle_path_candidate(
    candidates: Iterable[PathCandidate],
    *,
    prefer_source: str = "skeleton_contour",
) -> PathCandidate | None:
    viable = [candidate for candidate in candidates if candidate.reject_reason is None and candidate.score is not None]
    if not viable:
        return None
    preferred = [candidate for candidate in viable if candidate.source == prefer_source]
    pool = preferred if preferred else viable
    return max(pool, key=lambda candidate: float(candidate.score))
...
def _resolve_images(input_dir: Path, image: str | None) -> tuple[Path, ...]:
    ...
    selected = [
        ...
        if path.name == image or path.stem == image
    ]
    if selected:
...
def _coerce_svg_export_mode(value: str) -> SvgExportMode:
    return value  # type: ignore[return-value]


def _coerce_dxf_export_mode(value: str) -> DxfExportMode:
    return value  # type: ignore[return-value]
